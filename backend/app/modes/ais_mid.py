"""AIS MMSI → country (flag) lookup.

A ship MMSI is 9 digits; its first three are the ITU **Maritime Identification
Digits** (MID), which encode the country of registration. We map the MID to an
ISO-3166 alpha-2 code so the UI can show a flag and country name (it derives the
emoji + localized name from the ISO code, so we only keep the MID table here).

Only standard ship MMSIs (leading digit 2–7) carry the MID in the first three
digits; base stations / aids-to-navigation / SAR craft use other prefixes, so we
return ``None`` for those rather than guess.
"""
from __future__ import annotations

# MID (int) -> ISO 3166-1 alpha-2. Source: ITU Table of Maritime Identification
# Digits. Territories map to their flag's ISO code.
MID_TO_ISO2: dict[int, str] = {
...
def iso2_for_mmsi(mmsi: int | str) -> str | None:
    """Return the ISO-3166 alpha-2 country code for a ship MMSI, or None."""
    s = str(mmsi).strip()
    if len(s) != 9 or s[0] not in "234567":
        return None
    return MID_TO_ISO2.get(int(s[:3]))
